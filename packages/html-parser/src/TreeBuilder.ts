...
// ============================================================
enum InsertionMode {
  INITIAL = 'INITIAL',                   // 初始模式，处理 <!DOCTYPE>
  BEFORE_HTML = 'BEFORE_HTML',           // 在 <html> 之前
  BEFORE_HEAD = 'BEFORE_HEAD',           // 在 <head> 之前
  IN_HEAD = 'IN_HEAD',                   // 在 <head> 内部
  AFTER_HEAD = 'AFTER_HEAD',             // 在 <head> 之后，<body> 之前
  IN_BODY = 'IN_BODY',                   // 在 <body> 内部（最常用的模式）
  IN_TABLE = 'IN_TABLE',                 // 在 <table> 内部
  IN_TABLE_BODY = 'IN_TABLE_BODY',       // 在 <tbody>/<thead>/<tfoot> 内部
  IN_ROW = 'IN_ROW',                     // 在 <tr> 内部
  IN_CELL = 'IN_CELL',                   // 在 <td>/<th> 内部
  TEXT = 'TEXT',                         // 文本模式
}
...
export class HTMLTreeBuilder {
   ...
   */
  private originalInsertionMode: InsertionMode = InsertionMode.INITIAL;

  /**
   * 用于跟踪需要 foster parenting（领养）的元素
   */
  private fosterParenting: boolean = false;
  ...
  private processToken(token: HTMLToken): void {
    switch (token.type) {
      ...
      case TokenType.COMMENT:
        ...
        if (this.currentNode.nodeType !== NodeType.DOCUMENT_NODE) {
  ...
  // DOCTYPE 处理
  // ============================================================
  private processDOCTYPE(token: DOCTYPEToken): void {
  ...
  // 开始标签处理
  // ============================================================
  private processStartTag(token: StartTagToken): void {
  ...
  // BEFORE_HTML 模式
  // ============================================================
  private handleStartTagBeforeHTML(token: StartTagToken): void {
  ...
  // BEFORE_HEAD 模式
  // ============================================================
  private handleStartTagBeforeHead(token: StartTagToken): void {
    if (token.tagName === 'html') {
      // 已经被处理过了，忽略
      return;
  ...
  // IN_HEAD 模式
  // ============================================================
  private handleStartTagInHead(token: StartTagToken): void {
    const tagName = token.tagName;
  ...
  // AFTER_HEAD 模式
  // ============================================================
  private handleStartTagAfterHead(token: StartTagToken): void {
    const tagName = token.tagName;
  ...
  // ============================================================
  private handleStartTagInBody(token: StartTagToken): void {
    const tagName = token.tagName;
  ...
  // IN_TABLE 模式
  // ============================================================
  private handleStartTagInTable(token: StartTagToken): void {
    const tagName = token.tagName;
  ...
  // IN_TABLE_BODY 模式
  // ============================================================
  private handleStartTagInTableBody(token: StartTagToken): void {
    if (token.tagName === 'tr') {
  ...
  // IN_ROW 模式
  // ============================================================
  private handleStartTagInRow(token: StartTagToken): void {
  ...
  // IN_CELL 模式
  // ============================================================
  private handleStartTagInCell(token: StartTagToken): void {
    // 在 td/th 内部的元素
    this.insertElementAndPush(token);
  }
  ...
  // 结束标签处理
  // ============================================================
  private processEndTag(token: EndTagToken): void {
    const tagName = token.tagName;
   ...
   *   - 特殊元素不会被子元素的结束标签自动关闭
   */
  private handleEndTagInBody(tagName: string): void {
  ...
  // 字符处理
  // ============================================================
  private processCharacter(token: CharacterToken): void {
   ...
   */
  private insertElement(token: StartTagToken): IElement {
   ...
   */
  private insertElementAndPush(token: StartTagToken): IElement {
    const element = this.insertElement(token);
   ...
   */
  private createImpliedElement(tagName: string): void {
    if (!this.hasElementInStack(tagName)) {
   ...
   * 从开放元素栈弹出指定元素
   */
  private popElement(tagName: string): void {
    for (let i = this.openElements.length - 1; i >= 0; i--) {
   ...
   * 弹出直到（包括）指定元素
   */
  private popElementUntil(tagName: string): void {
    for (let i = this.openElements.length - 1; i >= 0; i--) {
      if (this.openElements[i].tagName.toLowerCase() === tagName) {
        this.openElements.splice(i, 1);
        break;
   ...
   * 检查开放元素栈中是否有指定标签
   */
  private hasElementInStack(tagName: string): boolean {
   ...
   */
  private handleTokenInTableWithFosterParenting(token: StartTagToken): void {
   ...
   * 获取最后插入的元素
   */
  private _lastInserted(): IElement | null {
