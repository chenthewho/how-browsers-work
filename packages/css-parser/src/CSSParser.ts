...
import { CSSTokenizer, CSSTokenType, type CSSToken } from './CSSTokenizer';
import { SelectorParser, type SelectorParseResult } from './SelectorParser';
...
export class CSSParser {
   ...
   */
  parse(cssText: string, origin: StyleOrigin = StyleOrigin.AUTHOR): StyleSheet {
   ...
   */
  private parseRule(): StyleRule | null {
    this.skipWhitespace();

    // @ 规则（暂跳过）
    if (this.current().type === CSSTokenType.AT_KEYWORD) {
      this.skipAtRule();
      return null;
    }
   ...
   */
  private parseDeclarationList(): CSSDeclaration[] {
    const declarations: CSSDeclaration[] = [];

    // 期待 {
    this.skipWhitespace();
    if (this.current().type !== CSSTokenType.OPEN_BRACE) {
      return declarations;
    }
    this.pos++; // 跳过 {
    ...
    while (!this.isEOF()) {
      ...
      // 遇到 } 结束
      if (this.current().type === CSSTokenType.CLOSE_BRACE) {
        this.pos++;
        break;
      }
   ...
   */
  private parseFunctionValue(): CSSValue | null {
    ...
    while (!this.isEOF() && this.current().type !== CSSTokenType.CLOSE_PAREN) {
    ...
    if (this.current().type === CSSTokenType.CLOSE_PAREN) {
      this.pos++; // 跳过 )
    }
   ...
   */
  private parseHexColor(hex: string): CSSValue {
    let h = hex.replace('#', '');
   ...
   */
  private resolveKeywordValue(keyword: string): CSSValue {
    // 已知颜色名映射
  ...
  private current(): CSSToken {
    return this.pos < this.tokens.length
      ...
      : { type: CSSTokenType.EOF, value: '' };
  }

  private isEOF(): boolean {
    return this.current().type === CSSTokenType.EOF;
  }
  ...
  private skipWhitespace(): void {
    while (this.current().type === CSSTokenType.WHITESPACE) {
      this.pos++;
    }
  }
   ...
   */
  private consumeUntilBrace(): string {
    let text = '';
    let depth = 0;
    ...
    while (!this.isEOF()) {
      ...
      if (token.type === CSSTokenType.OPEN_BRACE && depth === 0) {
        break;
      }
      if (token.type === CSSTokenType.OPEN_PAREN) {
        depth++;
      }
      if (token.type === CSSTokenType.CLOSE_PAREN) {
        depth--;
      }
  ...
  private skipAtRule(): void {
    ...
    while (!this.isEOF()) {
      ...
      if (token.type === CSSTokenType.OPEN_BRACE) braceDepth++;
      if (token.type === CSSTokenType.CLOSE_BRACE) {
        braceDepth--;
        if (braceDepth <= 0) {
          this.pos++;
          break;
      ...
      if (braceDepth === 0 && token.type === CSSTokenType.SEMICOLON) {
        this.pos++;
        break;
      }
 ...
 */
function extractNumber(v: CSSValue): number {
