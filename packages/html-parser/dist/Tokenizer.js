...
(function (State) {
    ...
    State["BEFORE_ATTR_NAME"] = "BEFORE_ATTR_NAME";
    State["ATTR_NAME"] = "ATTR_NAME";
    State["AFTER_ATTR_NAME"] = "AFTER_ATTR_NAME";
    State["BEFORE_ATTR_VALUE"] = "BEFORE_ATTR_VALUE";
    State["ATTR_VALUE_DOUBLE_QUOTED"] = "ATTR_VALUE_DQ";
    State["ATTR_VALUE_SINGLE_QUOTED"] = "ATTR_VALUE_SQ";
    State["ATTR_VALUE_UNQUOTED"] = "ATTR_VALUE_UQ";
    State["SELF_CLOSING_START_TAG"] = "SELF_CLOSING";
    State["MARKUP_DECL_OPEN"] = "MARKUP_DECL_OPEN";
    State["COMMENT_START"] = "COMMENT_START";
    State["COMMENT_START_DASH"] = "COMMENT_START_DASH";
    State["COMMENT"] = "COMMENT";
    State["COMMENT_END_DASH"] = "COMMENT_END_DASH";
    State["COMMENT_END"] = "COMMENT_END";
    State["DOCTYPE"] = "DOCTYPE";
    State["BEFORE_DOCTYPE_NAME"] = "BEFORE_DOCTYPE_NAME";
    State["DOCTYPE_NAME"] = "DOCTYPE_NAME";
    State["AFTER_DOCTYPE_NAME"] = "AFTER_DOCTYPE_NAME";
    State["RAWTEXT"] = "RAWTEXT";
    State["RAWTEXT_LESS_THAN"] = "RAWTEXT_LESS_THAN";
    State["RAWTEXT_END_TAG_OPEN"] = "RAWTEXT_END_TAG";
    State["RAWTEXT_END_TAG_NAME"] = "RAWTEXT_END_TAG_NAME";
    State["RCDATA"] = "RCDATA";
    State["RCDATA_LESS_THAN"] = "RCDATA_LESS_THAN";
    State["RCDATA_END_TAG_OPEN"] = "RCDATA_END_TAG";
    State["RCDATA_END_TAG_NAME"] = "RCDATA_END_TAG_NAME";
})(State || (State = {}));
class HTMLTokenizer {
    // 输入和位置
    input = '';
    ...
    static RAWTEXT_TAGS = new Set(['style', 'script', 'xmp', 'iframe', 'noembed', 'noframes']);
    static RCDATA_TAGS = new Set(['title', 'textarea']);
    /**
     * 执行词法分析，返回所有 token
     */
    tokenize(input) {
    ...
    handleTagOpen(ch) {
        if (ch === '!') {
            ...
            // 创建开始标签 token
            this.currentToken = (0, Token_1.createStartTagToken)(ch);
    ...
    handleEndTagOpen(ch) {
        if (isLetter(ch)) {
            this.currentToken = (0, Token_1.createEndTagToken)(ch);
            this.state = State.TAG_NAME;
        }
        else {
            // 解析错误：</ 后面不是字母
    ...
    handleMarkupDecl(ch) {
        if (ch === '-' && this.peek(1) === '-') {
            ...
            this.currentToken = (0, Token_1.createDOCTYPEToken)();
    ...
    handleRawtextEndTagName(ch) {
        if (isLetter(ch)) {
            ...
            if (endTagName === startTag) {
                // 成功！发射内容文本和结束标签
                this.flushCharacterBuffer();
                this.currentToken = (0, Token_1.createEndTagToken)(endTagName);
                this.emitCurrentToken();
                this.state = State.DATA;
            }
            else {
            ...
            if (this.buffer === this.rawtextEndTagName) {
                this.flushCharacterBuffer();
                this.currentToken = (0, Token_1.createEndTagToken)(this.buffer);
                this.state = State.BEFORE_ATTR_NAME;
            }
            else {
    ...
    processEOF() {
        ...
        // 发射 EOF token
        this.tokens.push((0, Token_1.createEndOfFileToken)());
    }
    // ============================================================
    // 辅助方法
    // ============================================================
    ...
    flushCharacterBuffer() {
        if (this.buffer.length > 0) {
            this.tokens.push((0, Token_1.createCharacterToken)(this.buffer));
            this.buffer = '';
        }
    }
    /**
    ...
    createCommentToken(data) {
        this.currentToken = (0, Token_1.createCommentToken)(data);
    }
    /**
...
function isStartTag(token) {
    return token.type === Token_1.TokenType.START_TAG;
}
/**
