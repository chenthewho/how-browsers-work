...
export class SelectorParser {
   ...
   * 例如：div.container > p + span
   */
  private parseComplexSelector(): ComplexSelector | null {
    const compounds: CompoundSelector[] = [];
    const combinators: Combinator[] = [];
    let specificity: SpecificityTuple = [0, 0, 0];
   ...
   */
  private readAttribute(): AttributeSelector | null {
    this.pos++; // 跳过 [
    this.skipWhitespace();
   ...
   */
  private readPseudoClass(): PseudoClass | null {
    this.pos++; // 跳过 :
  ...
  private readCombinator(): Combinator | null {
    const ch = this.current();
    if (ch === '>') { this.pos++; return 'child'; }
    if (ch === '+') { this.pos++; return 'adjacent-sibling'; }
    if (ch === '~') { this.pos++; return 'general-sibling'; }
    return null;
  }
   ...
   * 检查当前字符是否是组合器
   */
  private isCombinator(ch: string): boolean {
    return ch === '>' || ch === '+' || ch === '~';
  }
   ...
   * 检查是否到达选择器结束
   */
  private isEndOfSelector(): boolean {
    const ch = this.current();
    if (!ch) return true;
    if (ch === ',') return true;   // 选择器列表分隔符
    if (ch === '{') return true;  // 声明块开始
    return false;
  }
 ...
 * c: 类型/伪元素数量
 */
function compoundToSpecificity(compound: CompoundSelector): SpecificityTuple {
  let a = 0, b = 0, c = 0;
