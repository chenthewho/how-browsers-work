...
export interface CSSToken {
  type: CSSTokenType;
  value: string;       // token 的字符串表示
  numericValue?: number;// 数值（NUMBER/PERCENTAGE/DIMENSION）
  unit?: string;       // 单位（DIMENSION）
  // HASH token 的类型标记
...
export class CSSTokenizer {
   ...
   * 消费字符串 —— "..." 或 '...'
   */
  private consumeString(): CSSToken {
   ...
   */
  private consumeAtKeyword(): CSSToken {
    this.pos++; // 跳过 @
    let value = '';
   ...
   * 消费分隔符 —— 单个字符（: ; , { } ( ) [ ] 等）
   */
  private consumeDelim(): CSSToken {
    const ch = this.current();
    this.pos++;

    const DELIM_MAP: Record<string, CSSTokenType> = {
      ':': CSSTokenType.COLON,
...
function isIdentStart(ch: string): boolean {
  ...
  if (ch >= 'A' && ch <= 'Z') return true;
  if (ch.charCodeAt(0) > 127) return true; // 非 ASCII
  return false;
}
