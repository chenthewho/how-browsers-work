...
```
  ...
  │  包含块 (width: 300px)              │
  │                                    │
  │  Hello this is a long text that    │  ← 行1
  │  wraps to the next line here       │  ← 行2（放不下，换行）
  │  <span>inline box</span> more text │  ← 行3（行内元素嵌入文本流）
  │                                    │
  └────────────────────────────────────┘

  关键规则：
  • 文本和行内元素从左到右依次放置
  • 当剩余宽度不足以放下下一个单词/盒子时，换行
  • 每行高度 = max(该行所有盒子的高度)
  • 嵌套行内元素（如 <strong>）也参与同一行的流式排列
```
...
```
  ...
  │  │  div1   │                                            │
  │  └─ 20px ─┘  ← margin-bottom: 20px                     │
  │  ── 20px ──  ← 实际间距 = max(20, 10) = 20px             │
  │  ┌─ 10px ─┐  ← margin-top: 10px                        │
  │  │  div2   │    两个相邻块，margin 合并！                  │
  │  └─────────┘    不是 20+10=30，而是 max(20,10)=20        │
  └─────────────────────────────────────────────────────────┘
```
...
```
  ...
  Chrome DevTools 颜色约定（我们的可视化也遵循）：
    内容区 = 蓝色    padding = 绿色    border = 橙色    margin = 米色
```
